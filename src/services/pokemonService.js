...
const formatPokemonData = (
  ...
  return {
    ...
    color:
      species?.color?.name || 'gray',

    captureRate:
      species?.capture_rate || 0,
...
/**
 * Get a single, fully-formatted Pokemon
 */
export const getPokemonDetails = async (
  nameOrId
) => {
  const pokemon =
    await pokemonRepository.getPokemonByNameOrId(
      nameOrId
    );
  ...
  try {
    species =
      await pokemonRepository.getPokemonSpecies(
        pokemon.id
      );
  } catch {
...
export const searchPokemon = async (
  ...
  const searchResults =
    await pokemonRepository.searchPokemon(
      query
    );
...
/**
 * Get all selectable Pokemon types
 */
export const getPokemonTypes =
  async () => {
    const types =
      await pokemonRepository.getPokemonTypes();
...
/**
 * Get Pokemon filtered by type with pagination
 */
export const getPokemonByType =
  async (
    ...
    const pokemonList =
      await pokemonRepository.getPokemonByType(
        typeName
      );
